...
def getInitiaWalklNet():
    # choose link_types from 'motorway', 'trunk','primary','secondary', 'tertiary', 'residential'. default: 'all'
    net = og.getNetFromFile(filename=os.path.join(map_folder, 'map.osm'), network_types=('walk', 'bike'), default_lanes=True, default_speed=True, default_capacity=True)
...
def getInitialNet():
    # choose link_types from 'motorway', 'trunk','primary','secondary', 'tertiary', 'residential'. default: 'all'
    net = og.getNetFromFile(filename=os.path.join(map_folder, 'map.osm'), network_types=('auto', 'bike', 'walk'), POI=True, link_types=('motorway', 'trunk', 'primary', 'secondary', 'tertiary', 'residential'),default_lanes=True, default_speed=True, default_capacity=True)
...
def regenerateNet_based_on_macroNet():
    net = og.loadNetFromCSV(folder=map_folder,
                            node_file='node.csv', link_file='link.csv', movement_file='movement.csv')
