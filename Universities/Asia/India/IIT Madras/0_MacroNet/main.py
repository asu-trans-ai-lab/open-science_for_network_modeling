...
def generateDemand():
    gd.ReadNetworkFiles(map_folder)

    # Partition the area of interest into grid cells
    gd.PartitionGrid(number_of_x_blocks=None, number_of_y_blocks=None, cell_width=None, cell_height=None)

    gd.GetPoiTripRate(trip_purpose=1)
    gd.GetNodeDemand(residential_production=20, residential_attraction=20,
                     boundary_production=10, boundary_attraction=10)
    gd.ProduceAccessMatrix()
    gd.RunGravityModel(trip_purpose=1, a=None, b=None, c=None)

def findPath():
    network = pg.read_network(load_demand=True, input_dir=map_folder)
    print('\nshortest path (node id) from node 1410 to node 901, '
          + network.find_shortest_path(1410, 901, mode='all'))
    print('\nshortest path (link id) from node 1410 to node 901, '
          + network.find_shortest_path(1410, 901, mode='all', seq_type='link'))
    network.find_path_for_agents('all')
    pg.output_agent_paths(network, output_dir=map_folder)
...
if __name__ == '__main__':
    ...
    # Stage 4: generate travel demand
    generateDemand()

    # Stage 5: find travel paths
    findPath()
